...
class _WorkoutConfigurationScreenState extends State<WorkoutConfigurationScreen> {
  ...
  // State variables for the 5 questions
  String _fitnessLevel = 'Beginner';
  String _goal = 'Build Muscle';
  bool _useDumbbells = false;
  double _targetMinutes = 45.0;
  String _soreMuscleGroup = 'none at all'; // NEW STATE VARIABLE
  ...
  // NEW LIST OF OPTIONS
  final List<String> _soreMuscleOptions = [
    'none at all',
    'lower body',
    'upper body'
  ];
  ...
  Future<void> _generatePlan() async {
    ...
    try {
      ...
      final newPreference = WorkoutPreference(
        ...
        soreMuscleGroup: _soreMuscleGroup, // NEW: Include sore muscle group
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title:
            const Text('Generate New Plan', style: TextStyle(color: Colors.white)),
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // 4. Sore Muscle Group (NEW SECTION)
                  _buildSectionTitle('4. Sore Muscle Group?'),
                  _buildRadioGroup(_soreMuscleOptions, _soreMuscleGroup, (val) {
                    setState(() => _soreMuscleGroup = val!);
                  }),
                  const SizedBox(height: 15),

                  // 5. Minutes (Renumbered Section)
                  _buildSectionTitle('5. Target Duration: ${_targetMinutes.round()} mins'),
                  Slider(
                    value: _targetMinutes,
                    min: 15,
                    max: 60, // ⭐️ CORRECTED: Max duration set to 60 minutes
                    divisions: (60 - 15) ~/ 5, // steps of 5 minutes
